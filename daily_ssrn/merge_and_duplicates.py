...
def merge_categories(papers, ids):
    """
    合并论文类别信息，确保每篇论文只保留一个类别
    Merge paper category information, ensuring each paper retains only one category

    Args:
        papers (list): 论文数据列表 / List of paper data
        ids (set): 论文ID集合 / Set of paper IDs

    Returns:
        list: 合并后的论文数据列表 / Merged list of paper data
        set: 合并后的论文ID集合 / Merged set of paper IDs
    """
    # 用于聚合
    merged = {}
    ...
    for item in papers:
        ...
        if item_id not in merged:
            ...
            merged[item_id]['category'] = [item['category']]
        else:
            merged[item_id]['category'].append(item['category'])
    ...
    merged_ids = {paper['id'] for paper in merged_papers}

    return merged_papers, merged_ids
...
def perform_deduplication():
    ...
    try:
        ...
        today_papers, today_ids = merge_categories(today_papers, today_ids)
        print(f"今日论文合并后总数: {len(today_papers)} / Today's total papers after merging: {len(today_papers)}", file=sys.stderr)
        ...
        if duplicate_ids:
            print(f"发现 {len(duplicate_ids)} 篇历史重复论文 / Found {len(duplicate_ids)} historical duplicate papers",
                  file=sys.stderr)
            new_papers = [paper for paper in today_papers if paper.get('id', '') not in duplicate_ids]

            print(f"去重后剩余论文数: {len(new_papers)} / Remaining papers after deduplication: {len(new_papers)}",
                  file=sys.stderr)
...
def main():
    ...
    if dedup_status == "has_new_content":
        print("✅ 去重完成，发现新内容，继续工作流 / Deduplication completed, new content found, continue workflow",
              file=sys.stderr)
        sys.exit(0)
    elif dedup_status == "no_new_content":
        print("⏹️ 去重完成，无新内容，停止工作流 / Deduplication completed, no new content, stop workflow",
              file=sys.stderr)
        sys.exit(1)
